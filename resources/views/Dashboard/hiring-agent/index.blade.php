    ...
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card-box">
                        <a href="{{route('admin.hiring_agent.create')}}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="button-list">
                                            <a href="{{route('admin.hiring_agent.edit',$row->id)}}">
                                                <button class="btn btn-warning waves-effect waves-light"> <i class="fa fa-map-pin mr-1"></i> <span>تعديل</span> </button>
                                            </a>
                                            @if($row->status==1)
                                                <form class="ban" data-id="{{$row->id}}" method="POST" action="{{ route('admin.hiring_agent.ban',[$row->id]) }}">
                                                    @csrf
                                                    {{ method_field('POST') }}
                                                    <button class="btn btn-danger waves-effect waves-light"> <i class="fa fa-archive mr-1"></i> <span>حظر</span> </button>
                                                </form>
                                            @else
                                                <form class="activate" data-id="{{$row->id}}" method="POST" action="{{ route('admin.hiring_agent.activate',[$row->id]) }}">
                                                    @csrf
                                                    {{ method_field('POST') }}
