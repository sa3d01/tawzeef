...
{{--                                <th>التخصص الرئيسي</th>--}}
                            ...
                            @foreach($rows as $row)
                                <tr>
                                    <td>{{$row->name_ar}}</td>
{{--                                    <td>{{$row->parent?$row->parent->name_ar:""}}</td>--}}
...
{{--                                            </a>--}}
                                            @if($row->banned==0)
                                                <form class="ban" data-id="{{$row->id}}" method="POST" action="{{ route('admin.sector.ban',[$row->id]) }}">
                                                    @csrf
                                                    {{ method_field('POST') }}
                                                    <button class="btn btn-danger waves-effect waves-light"> <i class="fa fa-archive mr-1"></i> <span>حظر</span> </button>
                                                </form>
                                            @else
                                                <form class="activate" data-id="{{$row->id}}" method="POST" action="{{ route('admin.sector.activate',[$row->id]) }}">
                                                    @csrf
                                                    {{ method_field('POST') }}
